...
export class selectDBClass extends insertDbClass{
    ...
    selectDriverDetails = async (whereArr = [''], subquery = " `driver_id` != ?") => {
        const ql = "SELECT `driver_id`, `fname`, `mname`, `lname`, `email`, `phone`, `password`, `dob`, `gender`, `relationship`, `residence`, `park_area`, `vehicle_number`, `licence_number`, `tin_number`, `id_type`, `id_number`, `id_picture`, `passport`, `status`, `driver_date` FROM `drivers` WHERE "+subquery;
